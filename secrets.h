//Your WiFi credentials
const char *ssid = "OLIN-ROBOTICS";
const char *password = "R0B0TS-RULE";

//RTK2Go works well and is free
//const char casterHost[] = "rtk2go.com";
//const uint16_t casterPort = 2101;
//const char mountPoint[] = "bldr_dwntwn2"; //The mount point you want to push data to
//const char mountPointPW[] = "WR5wRo4H";

//Emlid Caster also works well and is free
const char casterHost[] = "caster.emlid.com";
const uint16_t casterPort = 2101;
const char casterUser[] = "u29226";
const char casterUserPW[] = "459yqs";
const char mountPoint[] = "MP23544"; //The mount point you want to push data to
const char mountPointPW[] = "287xrm";

